...
const Grid = (props) => {
  ...
  const getPenultimateDragDirection = (drag) => {
    ...
    else if (penultimateDrag % 4 === lastDrag % 4) return lastDrag;
  ...
  const checkIfFirstDrag = (drag) =>
    drag.filter((el) => el === null).length === 1;
  ...
  useEffect(() => {
    ...
    if (checkIfFirstDrag(drag)) {
      addSymbols([
        { index: drag[1], symbol: last },
        { index: drag[2], symbol: last },
      ]);
    } else {
      addSymbols([
        { index: drag[1], symbol: penultimate },
        { index: drag[2], symbol: last },
      ]);
    }
  }, [drag]);
  ...
  return <div className="grid">{genGrid(dim)}</div>;
};
